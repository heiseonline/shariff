...
| `data-facebooklike-css` | Name der CSS-Datei, die vom Dialog `facebooklike` verwendet werden soll. Die Datei muss im dem Ordner vorhanden sein, der mit der Option `data-dialogs-media-url` festgelegt wird. Beispiel : `data-facebooklike-css="my-styles.css"`. | `facebooklike_dlg.css` |
